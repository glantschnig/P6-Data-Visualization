...
d3.csv("baseball_data.csv", function(data) {
    ...
    footballPlayer = bubbles.addSeries(["name", "BMI", "handedness"], dimple.plot.bubble);
    footballPlayer.afterDraw = function(shape, data) {
        d3.select(shape).attr("r", "10");
    ...
    bubbles.assignColor("Right", "#008744", "#008744", 0.6);
    bubbles.assignColor("Left", "#0057e7", "#0057e7", 0.6);
    bubbles.assignColor("Both", "#ffa700", "#ffa700", 0.6);
    ...
    function activate(BMI) {
        // footballPlayer.shapes.transition().duration(frame / 2).style("visibility", function(d){
        //     return d.aggField[1] === BMI ? "visible" : "collapse";
        // });
        footballPlayer.data = dimple.filterData(data, "BMI", [BMI]);
        bubbles.draw();
